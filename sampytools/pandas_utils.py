...
from sampytools.list_utils import (
    construct_dict_from_list_of_key_values,
    reverse_list,
...
def strip_trailing_and_leading_spaces_from_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        try:
            ...
            logging.info(
                f"strip_trailing_and_leading_spaces_from_dataframe error on col {col}: {e}"
            )
            continue
    return df
...
def convert_columns_to_numeric(
    df: pd.DataFrame, numeric_columns: List[str], fill_na_val: float = 0.0
) -> pd.DataFrame:
    """
    ...
    for col in cols:
        if col in index:
            ...
            if col in diff_cols:
                ...
                df["diff_" + col + "_pct"] = (
                    df[col + suffixes[0]] / df[col + suffixes[1]] - 1
                ) * 100
                df["abs_diff_" + col] = abs(
                    df[col + suffixes[0]] - df[col + suffixes[1]]
                )
                df["abs_diff_" + col + "_pct"] = abs(
                    (df[col + suffixes[0]] / df[col + suffixes[1]] - 1) * 100
                )
